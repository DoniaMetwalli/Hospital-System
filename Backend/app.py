...
appointment_status = ['booked', 'rejected by hospital',
                      'rejected by doctor', 'canceled', 'fulfilled']
...
async def SignUp(patientInfo: PatientInfo, loginInfo: LoginInfo) -> int:
    try:
        ...
        data = (patientInfo.firstName, patientInfo.lastName, patientInfo.phone_number,
                patientInfo.gender, patientInfo.email, loginInfo.hashedUsername, loginInfo.hashedPassword)
        cnx.execute(query, data)
        conn.commit()
...
async def GetHospitalList(Name: str = None, City: str = None, Area: str = None) -> list[Hospital]:
    ...
    for result in results:
        Hospitals.append(Hospital(id=result[0], name=result[1], address=result[2],
                         phone_number=result[3], email=result[4], city=result[5], area=result[6]))
    conn.close()
    return Hospitals
...
async def GetAppointments(patient_id: int, status: str = "") -> list[AppendedAppointment]:
...
async def ChangeAppointment(appointment: Appointment) -> bool:
    try:
        ...
        data = (appointment.status, appointment.time, appointment.slot, appointment.dialysis_machine_id,
                appointment.hospital_id, appointment.doctor_id, appointment.appointment_id)
        conn = psycopg2.connect(**dbInfo)
        cnx = conn.cursor()
        cnx.execute(query, data)
        conn.commit()
...
async def EditUserInfo(loginInfo: LoginInfo, patientInfo: PatientInfo = None, doctorInfo: DoctorInfo = None) -> bool:
    try:
        ...
        if patientInfo != None:
            ...
            data = (patientInfo.firstName, patientInfo.lastName, patientInfo.email,
                    patientInfo.phone_number, patientInfo.gender, patientInfo.user_id)
            ...
            data = (doctorInfo.firstName, doctorInfo.lastName, doctorInfo.email,
                    doctorInfo.phone_number, doctorInfo.gender, doctorInfo.user_id)
...
async def AppendMedicalRecord(medicalRecordEntry: MedicalRecordEntry) -> bool:
    try:
        ...
        data = (medicalRecordEntry.appointment_id, medicalRecordEntry.patient_id,
                medicalRecordEntry.doctor_id, medicalRecordEntry.Diagnosis)
        conn = psycopg2.connect(**dbInfo)
        cnx = conn.cursor()
        cnx.execute(query, data)
        conn.commit()
...
async def AddDialysisMachine(dialysisMachine: DialysisMachine) -> bool:
    try:
        ...
        data = (dialysisMachine.hospital_id, dialysisMachine.startTime, dialysisMachine.time_slot,
                dialysisMachine.slotCount, dialysisMachine.price, dialysisMachine.availability)
        conn = psycopg2.connect(**dbInfo)
        cnx = conn.cursor()
        cnx.execute(query, data)
        conn.commit()
...
async def EditDialysisMachine(dialysisMachine: DialysisMachine) -> bool:
    try:
        ...
        data = (dialysisMachine.hospital_id, dialysisMachine.startTime, dialysisMachine.time_slot, dialysisMachine.slotCount,
                dialysisMachine.price, dialysisMachine.availability, dialysisMachine.dialysis_machine_id)
        conn = psycopg2.connect(**dbInfo)
        cnx = conn.cursor()
        cnx.execute(query, data)
        conn.commit()
...
async def AddDoctor(doctor: DoctorInfo, login: LoginInfo) -> bool:
    try:
        ...
        data = (doctor.firstName, doctor.lastName, doctor.phone_number,
                doctor.gender, doctor.email, login.hashedUsername, login.hashedPassword)
...
async def GetDoctors(hospitalID: int) -> list[DoctorInfo]:
    ...
    for result in results:
        doctors.append(DoctorInfo(user_id=result[0], firstName=result[1], lastName=result[2],
                       phone_number=result[3], gender=result[4], email=result[5], availability=True, hospital_id=hospitalID))
    return doctors
